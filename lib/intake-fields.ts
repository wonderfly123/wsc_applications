...
export const INTAKE_FIELDS: IntakeFieldDef[] = [
  ...
  { name: 'readyBy', label: 'Coconuts Ready at Service Start', type: 'number', required: true, clickupFieldId: '54389f19-e059-4418-b842-1b1cd12539ca', clickupFieldType: 'number', helpText: 'How many our staff should have opened and ready when guests arrive. The rest are opened on-demand during service.', section: 'package', half: true, hideWhenPackage: ['Sandcastle'] },
  ...
  { name: 'preOpenedQty', label: 'Coconuts Opened Before Transport', type: 'number', required: false, clickupFieldId: 'd2b20171-ebe0-4274-9100-a1de0e8ecbbe', clickupFieldType: 'number', helpText: 'How many to open at our kitchen and deliver already prepped. Leave 0 if none.', section: 'package', half: true },
  { name: 'preOpenedStyle', label: 'Pre-Opened Style', type: 'select', required: false, clickupFieldId: 'cf3f9f4b-4d43-4b8c-b6a9-c87fccad890a', clickupFieldType: 'drop_down', options: ['Straw opening', 'Cut open'], section: 'package', half: true, showWhen: { field: 'preOpenedQty', op: '>', value: 0 } },
  { name: 'preOpenedWaterSide', label: 'Coconut Water on the Side?', type: 'select', required: false, clickupFieldId: 'e5e7e431-989e-4808-9431-9609567f678c', clickupFieldType: 'drop_down', options: ['Yes', 'No'], helpText: 'Pour off and bottle the water separately (useful for cut-open style).', section: 'package', half: true, showWhen: { field: 'preOpenedQty', op: '>', value: 0 } },

  // === Event Location and Timing ===
  { name: 'setupTime', label: 'Set Up Date and Time', type: 'datetime-local', required: true, clickupFieldId: '', clickupFieldType: 'task_start_date', section: 'timing', half: true, labelByPackage: { Sandcastle: 'Delivery Window Start Date and Time' } },
  { name: 'teardownTime', label: 'Tear Down Date and Time', type: 'datetime-local', required: true, clickupFieldId: '', clickupFieldType: 'task_due_date', section: 'timing', half: true, labelByPackage: { Sandcastle: 'Delivery Window End Date and Time' } },
